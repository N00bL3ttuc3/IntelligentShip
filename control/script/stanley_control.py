...
class StanleyControlNode():
    def __init__(self):
        ...
        self.path_sub_ = rospy.Subscriber("finalpath", Path, self.callbackFromPath)
        self.pose_sub_ = rospy.Subscriber("boat_pose", PoseStamped, self.callbackFromPose)
        self.command_pub_ = rospy.Publisher("stanley_control_command",Command,queue_size=1)
        self.cubic_pub_ = rospy.Publisher("cubic_path",Path,queue_size=1)
        self.pose_pub_ = rospy.Publisher("target_pose",PoseStamped,queue_size=1)
        self.vertify_pub_ =  rospy.Publisher("path_vertify",Bool,queue_size=1)
        #trajectory path
        self.tx_ = []
        self.ty_ = []
        self.goal_ = []
    ...
    def callbackFromPath(self,path):
        ...
        # print("ty",self.ty_)
        #vertify
        self.path_flag.data=False
        self.vertify_pub_.publish(self.path_flag)
    ...
    def publishCommand(self):
        ...
        # self.command_.steer = -self.command_.steer
